...
const AdminLayoutContent = () => {
  ...
  return <div className="flex w-full min-h-screen bg-background">
      <Sidebar>
        ...
        <SidebarContent>
          <SidebarMenu>
            {adminNav.map(item => <React.Fragment key={item.title}>
                {!item.children ? <SidebarMenuItem>
                    <SidebarMenuButton asChild tooltip={item.title} isActive={item.exact ? location.pathname === item.path : location.pathname.startsWith(item.path)}>
                    ...
                    <SidebarGroupContent>
                      <SidebarMenu>
                        {item.children.map(child => <SidebarMenuItem key={child.path}>
                            <SidebarMenuButton asChild tooltip={child.title} isActive={location.pathname === child.path}>
        ...
        <div className="flex flex-col w-full">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              {state === "collapsed" && <Button variant="ghost" size="icon" className="lg:hidden" asChild>
                  <SidebarTrigger>
              ...
              <Breadcrumb>
                <BreadcrumbList>
                  {breadcrumbs.map((crumb, index) => <React.Fragment key={index}>
                      <BreadcrumbItem>
                        {crumb.path ? <BreadcrumbLink asChild>
                            <NavLink to={crumb.path}>{crumb.title}</NavLink>
                          </BreadcrumbLink> : <span>{crumb.title}</span>}
                      </BreadcrumbItem>
