...
serve(async (req) => {
  ...
  try {
    ...
    const { data: relevantEntries, error: searchError } = await supabase
      ...
      .select('title, content, updated_at, source_url')
    ...
    const sourceLinks = relevantEntries?.map(entry => ({
      title: entry.title,
      url: entry.source_url,
      lastUpdated: new Date(entry.updated_at).toLocaleDateString()
    })) || [];

    const knowledgeBaseContext = sourceLinks.length
      ? "\n\nVerified information from our knowledge base:\n" + 
        sourceLinks.map(source => 
          `${source.title} (Last Updated: ${source.lastUpdated})`
        ).join('\n\n')
      : '';
